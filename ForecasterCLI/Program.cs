...
namespace ForecasterCLI
{
    ...
    class Program
    {
        ...
        static int ParseAndExecute(string[] args, string secretApiKey = "")
        {
            ...
            train.Handler = CommandHandler.Create<string, string, int, int, bool, DateTime>(
                (symbol, quandlApiKey, horizon, seriesLength, toLatestData, startDate) =>
                {
                    ...
                    var data = Util.LoadPricesFromFile(datasetPath);
            ...
            predict.Handler = CommandHandler.Create<string, bool, string, float>((symbol, toLatestData, quandlApiKey, customPrice) =>
            {
                NLog.GlobalDiagnosticsContext.Set("symbol", symbol);
                if (string.IsNullOrEmpty(quandlApiKey))
                {
                ...
                var predictionData = predictor.Predict(customPrice);
                ...
                var datasetFile = Constants.DataDir + $"{symbol}_prediction_dataset.csv";

                Util.UpdateDataSetFile(symbol, datasetFile, predictionData.NewDataForModel, $"{symbol}_prediction_dataset.csv");
            });
